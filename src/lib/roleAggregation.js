...
export function aggregateByRole(tasks, hierarchy) {
  ...
  return hierList.map((row) => {
    ...
    for (const t of roleTasks) {
      console.log('[aggregateByRole] Task:', {
        role_aggregate: role_name,
        task_id: t.task_id,
        task_name: t.task_name,
        role_performing: t.role_performing,
        ai_allocation: t.ai_allocation,
        user_allocation: t.user_allocation,
      })
      ...
      console.log('[aggregateByRole] Final allocation:', finalAllocation)
      console.log('[aggregateByRole] Type of finalAllocation:', typeof finalAllocation)

      if (finalAllocation === 'tech-automated') {
        console.log('[aggregateByRole] Classified as lost')
        lost_tasks.push(t)
        lost_time_minutes += mins
      } else if (finalAllocation === 'human-only' || finalAllocation === 'tech-assisted') {
        console.log('[aggregateByRole] Classified as retained')
        retained_tasks.push(t)
        retained_time_minutes += mins
      } else {
        console.warn(
          '[aggregateByRole] Task fell into neither bucket!',
          t.task_name,
          finalAllocation,
        )
      }
